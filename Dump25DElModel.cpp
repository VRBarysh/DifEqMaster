...
cDump25DEl::cDump25DEl() : cEquBaseDump()
{
  DumpEls=0; Init.nSteps=Init.nEls=0; nXSteps=nTSteps=0; dx=0;
  hA=hI=NULL; hEl=NULL; hXProfileA=hXProfileB=NULL;
  DumpChanged=1;
}

void cDump25DEl::LoadInitData(cDifEqu_InitDataRec_25DElModel *hInit) {
  int nTStepsNew=hInit->tMax/hInit->dt;
  if( (Init.nSteps!=hInit->nSteps)||(hInit->nEls!=Init.nEls)||(nTStepsNew!=nTSteps))  {
    nTSteps=nTStepsNew;
    delete [] hI;  delete [] hA; delete [] hEl;
    hI=new TCplxLong[(hInit->nSteps+1)*nTSteps];
    hA=new TCplxLong[(hInit->nSteps+1)*nTSteps];
    hEl=new long double[(hInit->nSteps+1)*2*hInit->nEls*nTSteps];
...
void cDump25DEl::Dump(TCplxLong *hAData,TCplxLong *hIData,long double *hElData,int *hElPresDAta) {
  if(!DumpEnabled) return;
  if(Step>nTSteps-1) return;
  DumpReady=1;
  memcpy(hI+Step*(Init.nSteps+1),hIData,(Init.nSteps+1)*sizeof(TCplxLong));
  memcpy(hA+Step*(Init.nSteps+1),hAData,(Init.nSteps+1)*sizeof(TCplxLong));
  if(DumpEls) {
    memcpy(hEl+Step*(Init.nSteps+1)*2*Init.nEls,hIData,(Init.nSteps+1)*2*Init.nEls*sizeof(long double));
    memcpy(hElPresent+Step*(Init.nSteps+1)*Init.nEls,hIData,(Init.nSteps+1)*Init.nEls*sizeof(int));
  }
  Step++;
}

TCplxLong cDump25DEl::GetAVal(int ZStep,int XStep,int TStep) {
  TCplxLong dI,Res=0;
...
void cDump25DEl::MakeXProfileCoef(long double pdx, int pnXSteps) {
...
void cDump25DEl::DrawXProfileCoef(Graphics::TBitmap *hBitmap,cGr3DInitData Gr3DInit,DrawDumpInitData DrawInit){
  int tStep=DrawInit.t/Init.dt;
  tStep=tStep>Step?Step:tStep;
  long double SumA[10000],SumB[10000];
  if ( (DrawInit.dx!=dx) || (DrawInit.nXSteps!=nXSteps))
                         MakeXProfileCoef(DrawInit.dx,DrawInit.nXSteps);

  int iMaxA=0;
  for(int i=0;i<=Init.nSteps;i++) hI[i]=0;
  for(int i=0;i<=50;i++) hI[i]=TCplxLong((long double)rand()/RAND_MAX/Init.dt,0);
  for(int i=0;i<Init.nSteps;i++) {
    SumA[i]=SumB[i]=0;
    for(int j=0;j<nXSteps;j++) {
  //    SumA[i]+=abs(hXProfileA[i*nXSteps+j])*abs(hXProfileA[i*nXSteps+j]);
  //    SumB[i]+=abs(hXProfileB[i*nXSteps+j])*abs(hXProfileB[i*nXSteps+j]);
      long double tmp=abs(GetAVal(i,j,0));
      SumA[i]+=tmp*tmp;
    }
    if(SumA[i]>SumA[iMaxA]) iMaxA=i;
  }

  hBitmap->Canvas->Brush->Color=clWhite;
  hBitmap->Canvas->FillRect(TRect(0,0,640,480));
  hBitmap->Canvas->Pen->Color=clBlack;
  hBitmap->Canvas->MoveTo(0,120);
  hBitmap->Canvas->LineTo(640,120);
  hBitmap->Canvas->MoveTo(0,360);
  hBitmap->Canvas->LineTo(640,360);

  hBitmap->Canvas->MoveTo(0,120);
  for(int i=0;i<Init.nSteps;i++) {
    hBitmap->Canvas->LineTo(i*639/Init.nSteps,120-SumA[i]*Gr3DInit.ScaleZ);
  }
  /*
  hBitmap->Canvas->MoveTo(0,360);
  for(int i=0;i<Init.nSteps;i++) {
    hBitmap->Canvas->LineTo(i*639/Init.nSteps,360-SumB[i]*Gr3DInit.ScaleZ);
  } */
}

void cDump25DEl::DrawZSlice(Graphics::TBitmap *hBitmap,cGr3DInitData Gr3DInit,DrawDumpInitData DrawInit){
  int tStep=DrawInit.t/Init.dt;
  tStep=tStep>Step?Step:tStep;
  long double SumA[10000],SumB[10000];
  if ( (DrawInit.dx!=dx) || (DrawInit.nXSteps!=nXSteps))
...
void cDump25DEl::DrawZTSlice(Graphics::TBitmap *hBitmap,cGr3DInitData Gr3DInit,DrawDumpInitData DrawInit){
...
void cDump25DEl::DrawXTSlice(Graphics::TBitmap *hBitmap,cGr3DInitData Gr3DInit,DrawDumpInitData DrawInit){
  long double x1,y1,x2,y2,x3,y3,x4,y4,q1,q2;
  TPoint Poly[5];
...
void cDump25DEl::DrawXZSlice(Graphics::TBitmap *hBitmap,cGr3DInitData Gr3DInit,DrawDumpInitData DrawInit){
  long double x1,y1,x2,y2,x3,y3,x4,y4,q1,q2;
  TPoint Poly[5];
  int tStep=DrawInit.t/Init.dt;
  cGr3DMaster Gr3D=cGr3DMaster(Gr3DInit);
  ...
  if ( (DrawInit.dx!=dx) || (DrawInit.nXSteps!=nXSteps))
                     ...
                     // !*!*!*!*!*!**!@!!*!*!@*!*@*!@!**@#*!@*!*@*!*#*#*!@**!*#$*!@$*
  for(int i=0;i<=Init.nSteps;i++)
    hI[(Init.nSteps+1)*tStep+i]=0;
  hI[(Init.nSteps+1)*tStep+1]=TCplxLong(1.0/Init.dt,0);*/
...
void cDump25DEl::SaveCURR(char *hFileName){
...
void cDump25DEl::SaveZTSlice(DrawDumpInitData DrawInit) {
  double z,t;
  _fmode=O_BINARY;
  ...
  close(fSurface);
  fSurface=creat("ZTSliceI.dat", S_IWRITE);
  tmp1=0;
  for(int i=0;i<=Init.nSteps;i++){
    z=i*Init.dt;
    for(int j=0; (j<Step) ;j++) {
      t=j*Init.dt;
      tmpStr.sprintf("%G,%G,%G",(double)z,(double)t,(double)abs(hI[ (Init.nSteps+1)*j+i ]));
      tmp1=write(fSurface,tmpStr.c_str(),tmpStr.Length());
      tmp1=write(fSurface,fEoLine,2);
      tmp1=tmp1;
    }
...
void cDump25DEl::SaveXZSlice(DrawDumpInitData DrawInit) {
  double z,x;
  _fmode=O_BINARY;
  int tStep=DrawInit.t/Init.dt;
  if(tStep>Step-1) return;
...
void cDump25DEl::SaveXTSlice(DrawDumpInitData DrawInit) {
  double t,x;
  _fmode=O_BINARY;
  int zStep=DrawInit.t/Init.dt;
  if(zStep>Init.nSteps) return;
